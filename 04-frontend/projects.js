...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadProjectsFromBackend() {
        try {
            const res = await window.apiClient.get('/projects');
            if (res.ok && res.data && res.data.data) {
                projectsDatabase = res.data.data.map(p => ({
                    id: p.id,
                    name: p.name,
                    date: new Date(p.createdAt).toLocaleDateString(),
                    voice: "Aero", // Mocked since backend doesn't store this per project yet
                    lang: "English (US)",
                    duration: "0:15",
    ...
    const showMenuPopup = (project) => {
        ...
        document.getElementById('popDuplicate').addEventListener('click', async () => {
            try {
                const res = await window.apiClient.post('/projects', {
                    name: `${project.name} (Copy)`,
                    scriptText: project.rawText
                });
                if (res.ok) {
        ...
        document.getElementById('popRename').addEventListener('click', async () => {
            const newName = prompt(`Enter new project name for "${project.name}":`, project.name);
            if (newName && newName.trim() !== '') {
                try {
                    const res = await window.apiClient.put(`/projects/${project.id}`, {
                        name: newName.trim()
                    });
                    if (res.ok) {
        ...
        document.getElementById('popMp3').addEventListener('click', () => {
            ...
            let urlToOpen = project.audioUrl;
            // Fallback: If url is missing (like in old mock data), regenerate it on the fly!
            if (!urlToOpen && project.rawText) {
                const lang = project.lang === "Urdu" ? "ur" : "en";
                urlToOpen = `https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=${lang}&q=${encodeURIComponent(project.rawText.substring(0,200))}`;
            }
        ...
        document.getElementById('popDelete').addEventListener('click', async () => {
            if (confirm(`Are you sure you want to delete "${project.name}"?`)) {
                try {
                    const res = await window.apiClient.delete(`/projects/${project.id}`);
                    if (res.ok) {
