...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadVoicesFromBackend() {
        try {
            const res = await window.apiClient.get('/voices');
            if (res.ok && res.data && res.data.data) {
                // If backend returns data, map it. Otherwise fallback to mock DB.
                const backendVoices = res.data.data;
                if (backendVoices.length > 0) {
                    voicesDatabase = backendVoices.map(v => {
                        return {
                            name: v.name,
                            gender: (v.gender || "female").toLowerCase(),
                            lang: (v.lang || "en"),
                            accent: (v.accent || "us").toLowerCase(),
                            age: (v.age || "adult").toLowerCase(),
                            style: v.category || "Narration",
                            emotion: "calm",
                            avatarClass: "avatar-nova",
                            initials: v.name ? v.name.substring(0, 2).toUpperCase() : "VO",
                            desc: v.description || "High quality AI voice.",
                            isFavorite: v.featured || false,
                            sample: v.previewUrl || "",
                            popularity: 90
                        };
                    });
