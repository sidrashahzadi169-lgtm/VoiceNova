...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const drawWaveform = (mode) => {
        ...
        if (mode === 'idle') {
            ...
            // Draw rich overlapping voice synthesis curves
            waveOffset += 0.15;
            
            // Multiple curves with different properties to look organic
            const waves = [
            ...
            // Natural sound modulation simulation (so amplitude goes up and down over time)
            const volumeModulation = Math.sin(waveOffset * 0.05) * 0.5 + 0.5; // range 0 to 1
            const breathModulation = Math.max(0.1, Math.sin(waveOffset * 0.15)); // mimics word spacing pauses
            ...
            waves.forEach(wave => {
                ...
                ctx.moveTo(0, centerY);
                for (let x = 0; x < width; x++) {
                    // Create window envelope (attenuate edges of waveform to fit canvas nicely)
